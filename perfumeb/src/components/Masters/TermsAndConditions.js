...
export default function TermsAndConditions(props) {
  ...
  const validate = () => {
    ...
    temp.paraGraph = values.paraGraph === "" ? false : true;
    setErrors(temp);
  ...
  const applicationAPI = (url = "https://localhost:44373/api/termsandconditions/") => {
    return {
  ...
  const addOrEdit = (formData) => {
    applicationAPI()
      .update(formData.get("termsAndconditionsId"), formData)
      .then((res) => {
        handleSuccess("TermsAndconditions Updated");
        refreshTermsAndConditionsList();
      });
  };
  function refreshTermsAndConditionsList() {
    applicationAPI()
      .fetchAll()
      .then(
        ...
        <div className="col-sm-9 col-xs-12 content pt-3 pl-0">
          <form onSubmit={handleSubmit} autoComplete="off" noValidate>
            <span className="text-secondary">
              Dashboard <i className="fa fa-angle-right" /> Terms And Conditions
            </span>
            <div className="row mt-3">
              <div className="col-sm-12">
                <div className="mt-4 mb-3 p-3 button-container bg-white border shadow-sm">
                  <h6 className="mb-3">TermsAndConditions</h6>
