...
export default function ShippedOrders(props) {
    const [ordersList, setOrdersList] = useState([])
    const applicationAPI = (url = 'https://api.perfumatory.shop/api/order/') => {
        return {
            fetchAll: () => axios.get(url + 'GetOrdersByStatus/Dispatched')
        }
    }
        ...
        <div className="container-fluid">
            ...
            <div className="row main-content">
                ...
                <div className="col-sm-9 col-xs-12 content pt-3 pl-0">
                    <span className="text-secondary">Dashboard <i className="fa fa-angle-right" /> Dispatched List</span>
                                        ...
                                        <tbody>
                                            {ordersList && ordersList.map(order =>
                                                <tr key={order.orderId}>
                                                    ...
                                                    <td>
                                                        <Link className="btn btn-success mr-2" to={"/dispatchdetails/" + order.orderNo}><i className="fa fa-eye" /></Link></td>
                                                </tr>
                                            )}
                                        </tbody>
                                    </table>
